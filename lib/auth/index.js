...
function authorization(req) {
    ...
    return getSessionKey(code)
        .then(pkg => {
            ...
            // 存储到数据库中
            return AuthDbService.saveUserInfo(decryptedData, skey, session_key, user_agent, req.ip).then(userinfo => ({
...
function validation(req) {
    ...
    return AuthDbService.getUserInfoBySKey(skey)
        .then(result => {
            if (result.length === 0) { throw new Error(ERRORS.ERR_SKEY_INVALID) } else { result = result[0] }
            // 效验登录态是否过期
            const { last_visit_time: lastVisitTime, user_info: userInfo, session_key: sessionKey, status } = result
...
function validationMiddleware(ctx, next) {
    return validation(ctx.request).then(result => {
        ctx.state.$wxInfo = result
        return next()
    })
}
